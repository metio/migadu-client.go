...
func TestMigaduClient_GetMailbox(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "wildcard-sender-limits-and-timestamps",
			domain:    "example.com",
			localPart: "test",
			state: []model.Mailbox{
				{
					...
					IsActive:             true,
					WildcardSender:       true,
					DailyIncomingLimit:   100,
					WeeklyIncomingLimit:  200,
					MonthlyIncomingLimit: 300,
					DailyOutgoingLimit:   400,
					WeeklyOutgoingLimit:  500,
					MonthlyOutgoingLimit: 400,
					ActivatedAt:          "09/07/26",
					ChangedAt:            "26/07/26",
				},
			},
			want: &model.Mailbox{
